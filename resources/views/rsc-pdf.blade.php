...
<body>
    ...
    <div class="mb-8">
        ...
        <div class="mt-4">
            <div class="flex mb-4">
                <span class="font-semibold w-1/3 text-lg">PROJECT TITLE :
                    {{ strtoupper($rsc->team->thesis_title) }}</span>
            </div>
            <div class="flex mb-4">
                <span class="font-semibold w-1/3 text-lg">TEAM ALIAS : {{ strtoupper($rsc->team->name) }}</span>
            </div>
            <div class="flex mb-4">
                <span class="font-semibold w-1/3 text-lg">DATE :
                    {{ strtoupper(\Carbon\Carbon::parse($rsc->created_at)->format('F j, Y')) }}</span>
                &nbsp;&nbsp;&nbsp;&nbsp;
    ...
    <div class="overflow-x-auto relative mt-8">
        <table class="w-full text-sm text-left text-gray-500 border border-gray-300">
            <thead class="text-xs text-gray-700 uppercase bg-gray-100 border-b">
                <tr>
                    @if ($rsc->comment_for == $rsc::MANUSCRIPT)
                    ...
                    @if ($rsc->comment_for == $rsc::SYSTEM)
                        <th scope="col" class="py-3 px-6 border-r border-gray-300">Module No.</th>
                    @endif
    ...
    <div class="mt-10">
        ...
        <div class="grid grid-cols-3 gap-4">
            @foreach ($rsc->team->panelists as $panelist)
                <div>
                    <p class="mt-6 font-semibold">{{ strtoupper($panelist->name) }} <br>
                        {{ $panelist->is_panel_chair ? 'PANEL CHAIRMAN' : 'PANEL MEMBER' }}</p>
                </div>
            @endforeach
        </div>
    </div>
